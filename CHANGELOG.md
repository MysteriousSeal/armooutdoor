...
- **The guides took the shape of a document you consult.** Nobody reads a buying guide start to finish: someone arrives with one question, answers it, and comes back a month later with another. They were not laid out for that, being a stack of boxes and not-boxes, every part the same weight, with a short olive rule floating unattached above each title and a tinted toolbar of dot-separated links standing in for a plan. Every part is numbered now, and the number is what the plan points at; the rule that used to float is the top edge of that number, which is one mark doing two jobs instead of two doing one. Each part is a sheet of paper laid on the page, the way the shop already presents an article, so a guide reads as one document in several parts. The cards inside them take the page's own ground, because a white card on white paper is a hairline and nothing else. The plan is a row of pressable places rather than a toolbar, and the glossary's alphabet is now the same object: it had been a solid olive slab, which made the index the loudest thing in the guides and the entries under it the quietest.
- **The seven motifs are photographs of the cloth.** They were drawn in CSS gradients, which held their scale honestly and cost nothing to load but could only ever suggest a palette and the size of a blob, on a page whose entire subject is what a motif looks like. Each family now shows the weave, and each carries an alt describing the colours and the shape of the blobs, which is the pair of facts the guide itself says survives at twenty metres: someone who cannot see the photograph chooses on the same two facts as someone who can.

### Fixed

- **A shared link printed its apostrophes as code.** Every Open Graph tag on the site escaped its text a second time, so a link posted to a forum, a group or a chat drew a card reading "d&amp;#039;Armo Outdoor" where it should have read "d'Armo Outdoor". The framework escapes the string form of a section itself, which is why the page title and the description in the head are printed raw; the preview card was passing the same already-escaped content through the escaping helper again. This affected every page of the shop, not only the guides.
- **One guide's structured data could not be read at all.** The template language owns the at sign, so an unescaped context key compiles as a directive rather than as text. The camouflage guide shipped three blocks of structured data whose first key was a fragment of source code, and a block without a context is discarded: the article, the questions and the breadcrumb trail were all invisible to a crawler. The seven other guides escape it correctly and were never affected.
- **The questions were numbered twice on four guides**, because the panel wrapping them and the block inside it both counted as parts. Classification was the only guide that never printed its plan, and the only one built of four boxes with no open ground, so its table and its four legal chapters became parts in their own right. The glossary's index, filter and entries were loose children of the page and so never sat on a sheet at all. And two selector fieldsets stretched to equal height, leaving half a panel of white under the shorter one.

### Under the hood

- **The shelf is served from one stylesheet and one script.** There were eight of each, of which a single page used one plus the shared one: eight requests, and eight places to look for a rule. They are one file each now, fenced into named sections in the order the shelf presents them, and both sit beside the shop's own sheets rather than in a folder of their own. Grouping every rule by its declarations then turned up twelve things written out more than once, always the same object living in two files: the prose link, said five times; the uppercase micro-label, seven; the place cards and the segmented range of chips, written out identically by two guides each; the answer measure of the questions, by four; and the square of paper the cards are cut from, a surface and a hairline that twelve rules were each declaring for themselves. 2816 lines became 2681, and the design did not change: thirty seven computed properties and the box of every element under the main tag, on all eight pages, in both themes, snapshotted before and after, nineteen hundred and sixty six elements, none of them different.
...
- **The catalogue count lands in a target.** The figure was a statistic on a beige panel and the ground under it belonged to no trade in particular. Every discipline this shop equips is scored on concentric rings, so the count now sits in one — drawn at a fifth of the olive, masked to fade outward, cut by the panel's own edges. Glance at it and the numeral is still the only thing there; look at it and the numeral is in the ten. There is no filled centre: the first pass had one and it read as a smudge across the middle digit. Everything around the mark got quieter to pay for it — the numeral grows and the two counts beside it grow with it, having been dwarfed at the old size; the sentence stops being a caption, since at fifteen pixels of grey it sat under a headline that needs none, and weights its own second half, the range being a fact and where it ships from a promise; the dividers rule only the height of the lines they separate, where full-height rules drew a column holding nothing; and the button is sized like the way in rather than like a link.
- **The four promises under the hero are one band.** They were four white cards with a shadow each and a circle around every icon — the softest, roundest things on a page that squares everything and carries no shadow anywhere else, and four frames for a single row of facts. They are cells of one panel now, separated by the grid's own gap showing the ground through, which gives a hairline between neighbours at four columns, at two and at one without a divider having to know where it sits. The circles went with the cards: at the weight of the text beside it, the mark reads as part of the line. The titles come out of eight-hundred-weight capitals and back to sentence case at reading size, which is also what stops the longest of the four wrapping onto a second line while the other three sit on one. The threshold takes the olive stamp it already has in the band at the top of the page: of the four facts here it is the one being counted on.
- **Two of those promises now say something.** « Carte bancaire (PayPal bientôt) » announced a missing feature at the moment somebody decides whether to trust the shop, where naming Stripe borrows a name they already trust. « Vous êtes informé » promised nothing checkable — informed of what, and where — and the tracking number is on the order page, so that is what it says. Deliberately not « par e-mail »: no shipment mail is sent, and the stronger sentence would have been the false one. The headset goes with them, in both places it appeared, since the shop answers by mail and an icon offering a line nobody picks up is a promise broken at the first call.
- **« Pourquoi choisir Armo Outdoor ? » is set as a statement the shop signs.** It was four bordered cards inside a bordered panel, each with a circled icon and a number floating in its corner: frames within a frame. Two of the four also said what the strip under the hero already says, better and higher up — so they are gone, and in their place a fourteen-day return and the invoice, both of which the site actually honours and neither of which is said anywhere else. What is left is what the block always was. Nothing boxed and nothing ruled, the header ranged left with the clauses rather than centred above them. The numbering comes back as the article numbering of an undertaking, which is where a count from one to four finally says something true — how many promises there are, and that each is its own — and it is stencilled rather than printed, a solid figure that size having out-shouted the promise beside it. The question itself takes the olive and the highlighter stroke the other section titles wear, and the line under it drops a claim that the products were « testés, approuvés » in favour of how the catalogue is actually built. An ink stamp closes the sheet, in the corner opposite the heading on a wide screen and centred at the foot on a phone: a statement is signed, and a stamp is a mark the shop can make, where a handwritten signature would be a person the page cannot produce.
...
- **The dashboard can look at the whole shop.** Five periods, none longer than three months, could say what the last quarter did and never what the shop has done. « All time » joins them, opening at the first sale rather than at a round date — test orders excluded, or a stray one from two years back would open every chart on months the shop never lived. It has no earlier period and says so instead of inventing one: every delta reads as a dash, and the revenue chart drops its ghost line, its legend entry and the Previous column of the table beneath, a comparison series that is zero at every point being worse than none. Past four months the charts count by month; a hundred and fifty-five daily points is hair rather than a line, and a table twin of as many rows is not a reading either. Everything that names the bucket follows it, and the line stops smoothing, a curve between two months inventing a dip nobody measured.
- **The order pipeline wears the colours of the orders list.** Each stage now takes the tint its status already has on the list one arrives from — green shipped, violet in transit, blue delivered — rather than a second palette to learn for the same six words; a test holds each token against the badge it copies. Refunded joins them, last, since it is where an order leaves the queue rather than advances in it. The bar itself only scales what is still open: delivered stood at 185 of 211 and that share only grows, so it and refunded keep their swatch and their count under a rule, where they are totals rather than a queue. An empty queue says so in a line instead of drawing a bar over nothing.
- **A banned account stops being counted as a customer.** It leaves the shop accounts, the new customers, the buyers of the period, the returning and repeat shares, and the average spent per customer — that last one loses their orders too, since dividing everyone's money by the crowd that remains would flatter it. What stays is the revenue: that money really was taken, and dropping it would have the dashboard report less than the orders list for the same period.
- **The stock alerts say how much of the shortage is already on its way.** Sixty-two out of stock sent the reader to sixty-two product pages, eight of which needed nothing — the order was placed and only the postman was still owed. Both stock chips now carry that second figure, counting references rather than units, and reading the product's own purchase lines as well as its declinations'. A purchase order fully received stops counting, or the chip would go on promising a delivery that arrived last week.
- **Recent orders say what is in the box and where the sale came from.** How many items, and the marketplace's logo where one is uploaded, its name otherwise, and a word for the shop's own sales — written at the end of the facts rather than in a chip, six identical boxes reading « direct » having pushed each amount a different distance and broken the one column the eye follows down a list of prices.
- **Top products say what the unit sold for and what it cost to buy**, side by side, so the margin on a best seller reads without arithmetic; a cost is absent rather than zero when nothing has been received. The panel also gained its rank, which the row order was the only thing saying, and the share bar moved out from behind the product name onto its own rule beneath, where lengths compare down the column. The SKU sits under the name, that being what one reads aloud to a supplier, and the table no longer scrolls sideways inside its panel: the numbers keep their line and the title truncates.
- **The warehouse says what the shelves would fetch, not only what they cost.** Both figures stand at its head, at today's price with discounts applied. The margin between them covers only the references whose cost is known — the shelf value spans everything while the cost spans what has been received, and subtracting one from the other would have claimed three times the profit actually waiting there.
...
- **A sixth guide: where one may legally shoot.** At home, on an airsoft ground or at an approved range, with the question that actually decides drawn at the top of the page: the cone stops neither at the target nor at the fence, and what closes it is the point d'arrêt. The guide also says what each category may do and where. Category D at home under four conditions, at a range, never in a public place. Category C lawful on one's own ground in strict law and far more demanding in fact, since past twenty joules the backstop that held a pellet no longer holds. Category B at the range and nowhere else, the arm travelling unloaded and cased between the two.

- **Two texts the web copies backwards are corrected.** Article R. 312-40 is presented everywhere as permission to shoot at home: it governs sport shooting and does the opposite, reserving those arms to the ranges of the approved associations. The hundred and fifty metres are presented as a safety distance: they come from article L. 422-10 of the code de l'environnement, which withdraws from a communal hunt the land lying within that radius of a dwelling. The rule that does govern bears on the direction of the shot. Eight sources are listed and declared as citations, because a legal page nobody can check is worth nothing.

- **The joules and où-tirer guides can be read at a glance.** Each opens on its own plan, the conditions and the steps become numbered lists, cards say what is worth keeping, and the joules table finally explains its own shading.

### Under the hood

- **Each guide has its own stylesheet.** A single file had reached nineteen hundred lines and every guide loaded all of it, although only twenty-six of its hundred and thirty-one classes serve more than one page. The trunk keeps those, the rest lives beside it, one sheet per guide. The two oldest guides now load four hundred and sixty lines instead of nineteen hundred. The thousand and eight declarations of before are the thousand and eight of after, and for each of the seven pages the rules that apply to the classes it renders are the same as before.
...
- **The glossary goes from forty words to seventy-one, and the alphabet has no holes left.** Thirty-one terms of the shooting sport join the catalogue's vocabulary: la ligne de mire and its two sights, la dérive and l'élévation, le groupement, la détente and its break, le zérotage and la trajectoire that explains why a zero exists at one distance only, la portée utile which is not the maximum range, the four safety rules, le pas de tir, le cessez-le-feu, bench rest and plinking, le canon rayé, le carnet de tir, l'écouvillon, the ultrasonic bath, the windage engraved on the turret, the inner ten that scores nothing and breaks ties, and the yard that is enough to shift a zero when it is taken for a metre. All twenty-six letters now open something: an index that skips a letter sends the reader looking for what is missing rather than for what is there.

- **A word may now earn its definition without the shop selling anything for it.** An entry could only exist hooked to a rayon or to a filter. Twenty-five of them have nothing to sell and name instead the part of the sport they belong to, la visée, la technique, la sécurité, la balistique, la compétition, keeping the column's shape in the page's own greys rather than spending a colour they have not earned.

### Under the hood

- **An accented word now files under its plain letter.** The sort key folded accents, the initial did not: « Élévation » would have opened a group É after Z and left E empty, which is exactly the opposite of what an index is for.
...
- **A fifth guide: the glossary.** Forty words from the shelves, defined one by one: AEG, hop-up, MED, diabolo, QD, M-LOK, MOLLE, paracorde, keffieh, grille graduée, témoin de chambre vide, and the camouflage sigils eighty-five products wear without the site ever having said what « CP » meant. It is not a dictionary but a legend for the catalogue: every entry says where the word is met in the shop and leads there. Nine of them land on a rayon already filtered, with the number of products answering to that filter today; the rest on a rayon, a guide or an article. A filter that finds nothing, or a rayon this shop does not carry, keeps its definition and loses its link rather than leading to an empty shelf.

- **The glossary reads like an inventory sheet.** The word and its definition on the left, the place it lives on the right, in a column of the same width from end to end: the numbers line up instead of trailing behind forty paragraphs. Each destination wears the colour of what it is, stock you can filter, a shelf, or what the shop has written, in the palette the catalogue already uses to mean something. The letter follows its group like the thumb tab of a dictionary, and the index shows the whole alphabet, spent letters included: an index that quietly skips G reads as broken. The search field ignores accents and knows the synonyms, but it only appears if the page has JavaScript; the list and the index work without it.

- **A link already broken in production is repaired.** The pastilles rayon had been renamed, and the glossary pointed at the old address. All twenty-two rayons it cites were checked one by one.

### Under the hood

- **A test failed one run in a hundred, with nothing broken.** It looked for « 1a » in the whole page of the cibles guide, and the CSRF token in the head is forty letters and digits drawn at random: measured over two hundred thousand draws, such a token carries « 1a » 0.99 % of the time. The test now reads the guide rather than the page around it.

- **The guides sitemap follows the shelf.** The glossary enters it without anyone having to write it in.
...
- **The terrain limits are named on the joules and FPS guide's axis.** The three marks carried only a tooltip, and the text below never said those scratches were the limits it was describing: three unreadable ticks. They sit within seven per cent of the axis, so a single brace names all three rather than three labels fighting for the same sliver of rule.
...
- **A fourth guide: joules and FPS.** The shop quotes FPS, the law counts in joules and so do the grounds, and nothing on the site joined the two. The guide carries the formula, a calculator taking a bille and a speed in either unit, a table of forty-nine conversions computed rather than copied, the reasons a single replica never chronographs the same way twice, and five questions. The whole page rests on an axis from 0.08 to 20 joules, logarithmic because a linear one would crush everything an airsoft player cares about into its first two per cent: the calculator walks a needle along it, and the terrain limits line up just short of the legal mark, where they actually are. The table darkens as it approaches 2 joules and flags the cells that have crossed. Without JavaScript the calculator does not appear at all and the table answers in its place.

### Under the hood

- **The sitemap reads the shelf instead of copying it.** The guides were listed there by hand: one guide more and it was missing from the sitemap with nothing to say so. It now walks the same list as the index page and the home page.
...
- **Google Analytics, behind the same gate as PostHog.** The tag is not fetched and nothing is sent until the visitor accepts; refusal and silence both mean the script is never requested. Advertising signals and personalisation are off and the address is anonymised, this being measurement rather than advertising. Google is a United States processor, so the banner names it apart from the one hosted in the Union, and the privacy policy states the basis for the transfer, the adequacy decision of 10 July 2023, rather than leaving a reader to assume there is one. The Content-Security-Policy now gathers its hosts from whichever tools are configured instead of knowing about a single one; with neither configured it is the closed policy it always was.

- **The shop's events, said in the vocabulary Google reads.** GA4 reserves `add_to_cart`, `begin_checkout` and `purchase`, files anything else as a counter that ticks and never reaches a revenue report, and reads « items » as a list of products where the shop was sending a count: so the install would have been decorative without this. Both vocabularies are spoken from one call site now: PostHog keeps the shop's own names and figures, Google takes its reserved ones with the products written out, priced and counted. The purchase carries a transaction id, without which a customer refreshing the confirmation page books the sale a second time, and its lines are read from the order's own rows rather than from the products they point at, since renaming or repricing a product after a sale must not change what the sale is reported to have been.

- **Each test is fed only the arguments it takes.** Four of the layout and breadcrumb tests declared one parameter while their data provider handed over two. PHPUnit does not fail on that, it warns, so the suite came out green and seven warnings went by unread.

**No migration.** The measurement id is set in the environment: production needs `GA_MEASUREMENT_ID` in its own `.env` and the config cache rebuilt.
